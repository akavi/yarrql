...
function toSql(q) {
    ...
    const ctx = { aliasGen, depth: 0, aliasMap: new Map() };
...
function emitQuery(q, ctx) {
    switch (q.type) {
        ...
        case "limit": {
            const { sql: srcSql, alias } = emitQuery(q.source, ctx);
            const limitSql = emitExpr(q.limit, alias, ctx);
            return {
        ...
        case "offset": {
            const { sql: srcSql, alias } = emitQuery(q.source, ctx);
            const offsetSql = emitExpr(q.offset, alias, ctx);
            return {
        ...
        case "flat_map": {
            const { sql: sourceSql, alias: srcAlias } = emitQuery(q.source, ctx);
            const { sql: flatMapSql, alias: fmAlias } = emitQuery(q.flatMap, ctx);
            const alias = ctx.aliasGen.next().value;
            return {
...
function emitExpr(e, tableAlias, ctx) {
    switch (e.type) {
        ...
        case "row": {
            ...
            const alias = ctx.aliasMap.get(e.source) ?? tableAlias;
            return `${alias}.*`;
        }
        case "first": {
            const srcSql = emitExpr(e.source, tableAlias, ctx);
        ...
        case "count": {
            const { sql: srcSql } = emitQuery(e.source, ctx);
